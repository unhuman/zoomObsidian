...
**Incremental sync:** Shared meetings use a `lastProcessedShared` date stored in `obsidian-config.json`. On the next run, `listSummaries()` receives a `from` date so only new shared meetings are fetched. The shared scan date is advanced only when **both** `autoDelete` is on **and** no topic filter is active — the stored date is the latest meeting date seen (not the run timestamp). When a **topic filter** is active, the shared scan date is **not** advanced — this prevents non-matching meetings from being permanently skipped on subsequent unfiltered runs. Owned meetings always do a full scan (no timestamp tracking) because they can be deleted from Zoom after writing. Delete `obsidian-config.json` to force a full re-scan of shared meetings.
